...
const TESTIMONIALS = [
  { name: "Fatima Z.", role: "Patiente, Casablanca", avatar: "FZ", color: "#0057B8", rating: 5, textKey: "t1_text" },
  { name: "Dr. Karim B.", role: "Médecin Généraliste, Rabat", avatar: "KB", color: "#0891b2", rating: 5, textKey: "t2_text" },
  { name: "Youssef A.", role: "Patient, Marrakech", avatar: "YA", color: "#7c3aed", rating: 5, textKey: "t3_text" },
  { name: "Nadia M.", role: "Patiente, Agadir", avatar: "NM", color: "#d97706", rating: 5, textKey: "t4_text" },
  { name: "Dr. Samira H.", role: "Cardiologue, Fès", avatar: "SH", color: "#dc2626", rating: 5, textKey: "t5_text" },
  { name: "Omar T.", role: "Patient, Tanger", avatar: "OT", color: "#0d9488", rating: 5, textKey: "t6_text" },
];
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            ...
            <div style={{ background: "#fff", border: "2px solid #fecaca", borderRadius: 20, padding: 28 }}>
              ...
              {[1,2,3,4,5,6].map(i => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderBottom: "1px solid #fef2f2" }}>
                  <span style={{ color: "#dc2626", fontSize: 16, flexShrink: 0 }}>✕</span>
                  <span style={{ fontSize: 14, color: "#64748b" }}>{t(`landing.without${i}`)}</span>
                </div>
              ))}
            </div>
            {/* With */}
            <div style={{ background: "#fff", border: "2px solid #bbf7d0", borderRadius: 20, padding: 28 }}>
              ...
              {[1,2,3,4,5,6].map(i => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderBottom: "1px solid #f0fdf4" }}>
                  <span style={{ color: "#16a34a", fontSize: 16, flexShrink: 0 }}>✓</span>
                  <span style={{ fontSize: 14, color: "#374151", fontWeight: 500 }}>{t(`landing.with${i}`)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 20 }}>
            {TESTIMONIALS.map((item) => (
              <div key={item.name} style={{ background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: 20, padding: 24, transition: "box-shadow 0.2s", display: "flex", flexDirection: "column", gap: 16 }}
                ...
                <p style={{ fontSize: 14, color: "#374151", lineHeight: 1.7, margin: 0, flex: 1 }}>"{t(`landing.${item.textKey}`)}"</p>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: item.color + "18", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 13, color: item.color, flexShrink: 0 }}>
                    {item.avatar}
                  </div>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", margin: 0 }}>{item.name}</p>
                    <p style={{ fontSize: 12, color: "#94a3b8", margin: 0 }}>{item.role}</p>
                  </div>
                </div>
              </div>
            ))}
            ...
            {FAQS.map((faq, i) => (
              <div key={i} style={{ background: "#f8fafc", border: `1.5px solid ${openFaq === i ? "#0057B8" : "#e5e7eb"}`, borderRadius: 14, overflow: "hidden", transition: "border-color 0.2s" }}>
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 20px", background: "none", border: "none", cursor: "pointer", textAlign: "left", gap: 12 }}>
                  <span style={{ fontSize: 15, fontWeight: 700, color: "#0f172a", flex: 1 }}>{t(`landing.${faq.qKey}`)}</span>
                  {openFaq === i ? <ChevronUp size={18} color="#0057B8" style={{ flexShrink: 0 }} /> : <ChevronDown size={18} color="#94a3b8" style={{ flexShrink: 0 }} />}
                </button>
                {openFaq === i && (
                  <div style={{ padding: "0 20px 18px" }}>
                    <p style={{ fontSize: 14, color: "#64748b", lineHeight: 1.7, margin: 0 }}>{t(`landing.${faq.aKey}`)}</p>
                  </div>
                )}
              </div>
            ))}
      ...
      <footer style={{ background: "#0a1628", padding: "48px 16px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 40, marginBottom: 40, flexWrap: "wrap" }}>
            ...
            <div>
              <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 16px" }}>{t("landing.footer_doctors")}</p>
              {[[t("landing.footer_join_doctor"), "/register"], [t("landing.footer_dashboard"), "/dashboard"], [t("landing.footer_appointments"), "/dashboard"]].map(([label, href]) => (
                <Link key={label} to={href} style={{ display: "block", color: "rgba(255,255,255,0.45)", fontSize: 14, textDecoration: "none", marginBottom: 10, transition: "color 0.15s" }}
                  onMouseEnter={e => (e.currentTarget.style.color = "#fff")}
                  onMouseLeave={e => (e.currentTarget.style.color = "rgba(255,255,255,0.45)")}>
                  {label}
            ...
            <div>
              <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 16px" }}>{t("landing.footer_cities")}</p>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 24, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 20 }}>
              {[t("landing.footer_privacy"), t("landing.footer_terms"), t("landing.footer_contact")].map(l => (
