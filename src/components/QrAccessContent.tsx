...
interface QrToken { id: string; patientId: string; expiresAt: number; used: boolean; }

export function QrAccessContent() {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (remaining === 0) { clearInterval(intervalRef.current!); setToken(null); }
    }, 1000);
  ...
  const downloadQr = () => {
    ...
    a.href = url; a.download = `spitar-qr-${user?.spitarId}.svg`; a.click();
  ...
  const printQr = () => {
    ...
    if (!win) return;
    win.document.write(`<html><head><title>SPITAR QR - ${user?.spitarId}</title></head>
  ...
  const qrUrl = token ? `${window.location.origin}/qr-scan/${token.id}` : "";
  ...
  return (
    <div className="max-w-md mx-auto py-4">
      <div className="text-center mb-6">
        ...
        <h1 className="text-xl font-bold">{t("dashboard.qr_access") || "Emergency QR Access"}</h1>
